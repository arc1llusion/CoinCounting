...
int main() {
	...
	pinMode(RST, OUTPUT); //LCD reset and data/command pin are outputs on GPIO pins
	pinMode(DC, OUTPUT);
...
void LCDinit(void) {
	...
	digitalWrite(RST, RESET_LOW);
	for(delay = 0; delay < 10; delay = delay + 1);
	digitalWrite(RST, RESET_HIGH);
...
void LCDwrite(enum typeOfWrite type, unsigned char message) {
	...
	if(type == COMMAND) {
		digitalWrite(DC, DC_COMMAND); //Set data/command to command mode
		...
		digitalWrite(DC, DC_DATA); //Set data/command to data mode
		wiringPiSPIDataRW(CHANNEL, buffer, 1); //Wait a bit
	}
}
